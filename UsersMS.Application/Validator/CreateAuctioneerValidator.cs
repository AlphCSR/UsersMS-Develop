...
{
    public class CreateAuctioneerValidator : ValidatorBase<CreateAuctioneerDto>
    {
        public CreateAuctioneerValidator()
        {
            ...
            RuleFor(s => s.Name).NotEmpty().WithMessage("Name no puede estar vacío").WithErrorCode("041");
            RuleFor(s => s.LastName).NotNull().WithMessage("Apellido no puede ser nulo").WithErrorCode("050");
            RuleFor(s => s.LastName).NotEmpty().WithMessage("Apellido no puede estar vacío").WithErrorCode("051");
            RuleFor(s => s.Id).NotNull().WithMessage("Cedula no puede ser nula").WithErrorCode("030");
            RuleFor(s => s.Id).NotEmpty().WithMessage("Cedula no puede estar vacío").WithErrorCode("031");
            ...
            RuleFor(s => s.Address).NotEmpty().WithMessage("Direccion no puede estar vacío").WithErrorCode("091");
