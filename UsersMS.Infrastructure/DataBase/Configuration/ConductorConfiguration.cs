...
namespace UsersMS.Infrastructure.DataBase.Configuration
{
    public class ConductorConfiguration : IEntityTypeConfiguration<Conductor>
    {
        public void Configure(EntityTypeBuilder<Conductor> builder)
        {
            ...
            builder.Property(s => s.Licencia).IsRequired();
            builder.Property(s => s.CertificadoSalud).IsRequired();
            builder.Property(s => s.DepartamentoId);
            builder.Property(s => s.EmpresaId);
            builder.Property(s => s.GruaId);//al no colocarle nada estoy diciendo que no son requeridos, me permite crear un administrador sin tener que darle estos atributos,
